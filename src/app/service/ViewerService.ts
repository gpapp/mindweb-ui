...
export default class ViewerService implements BroadcastListener {
    ...
    public onMessage(message: AbstractMessage): void {
        if (message instanceof AbstractBroadcast) {
            ...
            if (response instanceof EditResponse) {
                MapService.applyAction(this._currentMapVersion.value.content, response.action,
                    (error) => {
                        if (error) {
                            //TODO: handle error
                            return;
    ...
    public sendEditAction(event: ActionNames, id: string, payload: any) {
        const editAction = new EditAction();
        editAction.parent = id;
        editAction.event = event;
        editAction.payload = payload;
        this.websocketService.sendMessage(
            new EditRequestImpl(this._currentMapVersion.value.container.id, editAction),
